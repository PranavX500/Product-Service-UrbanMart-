...
public class ProductConsumer {
    ...
    public void handleIds(final ListSuccessEvent event) {
        Map<Long, Integer> quantityMap = event.getItems().stream().collect(
                Collectors.toMap(ItemQuantityDTO::getId, ItemQuantityDTO::getQuantity)
        );
        List<Long> ids = event.getItems().stream().map(ItemQuantityDTO::getId).toList();
        ...
        List<ProductDto> products = productList.stream()
                .map(product -> new ProductDto(
                        ...
                        product.getPrice(),
                        quantityMap.getOrDefault(product.getId(), product.getQuantity()),
